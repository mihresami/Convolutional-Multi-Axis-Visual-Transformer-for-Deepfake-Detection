...
# from Cvit import Residual,PreNorm
...
class PartitionAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        self.G_i = nn.Parameter(torch.randn(in_channels,grid_size[0],grid_size[1]))  # Random initialization with same shape
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Apply 2D FFT
        x1 = torch.fft.fftn(x1, dim=(-2, -1))
        
        # # Hadamard product
        x1 = self.G_i * x1

        # # Apply inverse FFT
        x1 = torch.fft.ifftn(x1, dim=(-2, -1)).real
        x1 += x


        x = self.partition_op(x1, self.p)
...
class MaxVit(nn.Module):
    ...
    def __init__(
        ...
        # Additional layer for learning relationship between local and global features
        self.resnet = resnet18(weights='DEFAULT')
        for param in self.resnet.parameters():
            param.requires_grad = False
        inp_feat = self.resnet.fc.in_features
        self.resnet.fc = nn.Linear(in_features=inp_feat,out_features=block_channels[-1])
        # self.res_global = nn.Linear(block_channels[-1],block_channels[-1])
        # self.local = nn.Linear(block_channels[-1],block_channels[-1])
        self.mlp = nn.Sequential(
            nn.Linear(block_channels[-1]*4, block_channels[-1]),
            nn.GELU(),
            nn.Dropout(p=0.5),
            nn.Linear(block_channels[-1], 2),
        )  
        self.linear = nn.Linear(block_channels[-1],2)
        self.feature = None
        self.linear = nn.Linear(512,2)
        # self.mlp1 = nn.Sequential(
        #     nn.Linear(block_channels[-1]*2, block_channels[-1]),
        #     nn.GELU(),
        #     nn.Dropout(p=0.5),
        #     # nn.Linear(block_channels[-1], 512),
        # )  
        # self.dim = 512*2
        # self.mlp_dim = self.dim * 2
        # self.heads = 8
        # self.depth = 6
        # self.cls_token = nn.Parameter(torch.randn(1, 1, self.dim))
        # self.transformer = Transformer(self.dim, self.depth, self.heads, self.mlp_dim)
        # self.pos_embedding = nn.Parameter(torch.empty(1, 2, self.dim).normal_(std=0.02))



        # self._init_weights()
        self.G_i2 = nn.Parameter(torch.randn(512))  # Random initialization with same shape
        self.G_i = nn.Parameter(torch.randn(3,224,224))  # Random initialization with the orginal shape
        
    # this forward for frequency starts after resnet global = 512 and For mvit 2D FF will calcuated inside attention for all
    # def forward(self, x: Tensor) -> Tensor:
    #     f_i = x
    #     f_i = torch.fft.fftn(f_i, dim=(-2,-1))
    #     # Hadamard product
    #     f_i = self.G_i * f_i
    #     # # Apply inverse FFT
    #     f_i = torch.fft.ifftn(f_i, dim=(-2, -1)).real

    # #     resnet_global = self.resnet(x)
    # #     # f_i = resnet_global
    # #     # # Apply 2D FFT
    # #     # f_i = torch.fft.fftn(f_i, dim=(-1))
    # #     # # Hadamard product
    # #     # f_i = self.G_i * f_i
    # #     # # # Apply inverse FFT
    # #     # f_i = torch.fft.ifftn(f_i, dim=(-2, -1)).real
    ...
    #         x = block(x)
    #     x = self.classifier(x)
    #     x = self.mlp(torch.cat((x,resnet_global),dim=1))
    #     return x
    #frequency is orginal for both. 3,224,224 This par includes local and global ones 
    # def forward(self, x: Tensor) -> Tensor:
    ...
    #     # f_i = torch.fft.ifftn(f_i, dim=(-2, -1)).real
    #     resnet_global = self.resnet(x)
    #     # classifier = resnet_global
    #     # f_i_global = resnet_global
    #     # # Apply 2D FFT
    #     # f_i_global = torch.fft.fftn(f_i_global, dim=(-1))
        
    #     # # # Hadamard product
    #     # f_i_global = self.G_i2 * f_i_global 
    #     # # # Apply inverse FFT
    #     # f_i_global = torch.fft.ifftn(f_i_global, dim=(-1)).real
    #     x = self.stem(x)
    #     for block in self.blocks:
    #         x = block(x)
    #     x = self.classifier(x)
    #     # f_i_local = x
    #     # f_i_local = torch.fft.fftn(f_i_local, dim=(-1))
        
    #     # # # Hadamard product
    #     # f_i_local = self.G_i2 * f_i_local
    #     # # # Apply inverse FFT
    #     # f_i_local = torch.fft.ifftn(f_i_local, dim=(-1)).real

    # #    
    #     # x = self.mlp(torch.cat((x,resnet_global,f_i_local,f_i_global),dim=1))
    #     x = self.mlp(torch.cat((x,resnet_global),dim=1))
        # feat = x
        # x = self.linear(x)
    ...
    #     x = torch.cat((x+f_i_local, resnet_global+f_i_global),dim=1).unsqueeze(dim=1)
    ...
    #     return x
    def forward(self,x):
    #global and local frequency from both models and frquency is in all mvit but not in (stem) 
        f_i = x
        # Apply 2D FFT
        f_i = torch.fft.fftn(f_i, dim=(-2,-1))
        ...
        # # Apply inverse FFT
        f_i = torch.fft.ifftn(f_i, dim=(-2, -1)).real
        resnet_global = self.resnet(x+f_i)
        f_i_global = resnet_global
        # Apply 2D FFT
        f_i_global = torch.fft.fftn(f_i_global, dim=(-1))
...
#         # # Hadamard product
#         f_i_global = self.G_i * f_i_global
#         # # Apply inverse FFT
#         f_i_global = torch.fft.ifftn(f_i_global, dim=(-1)).real
#         x = self.stem(x)
#         for block in self.blocks:
...
#         x = self.classifier(x)
        ...
        # x = self.mlp(torch.cat((x,resnet_global,f_i_local,f_i_global),dim=1))
        # x = self.mlp(torch.cat((x,resnet_global,f_i_global),dim=1))
        # return x
#     def forward(self,x):
#         f_i = x
#         # Apply 2D FFT
...
# model = maxvit_t()
# # model.load_state_dict(torch.load('weight_all_data/Deep_model_FF++_No_Freq-29-acc0.9194738460187096.pt'),strict=False)
